...
import { HttpClient, MessageSigner, Authenticator, PortfolioFetcher, Position, ConsoleLogger } from "@limitless/exchange-ts-sdk";
...
function displayPosition(position: Position, index: number) {
  ...
  console.log(`   Market Status: ${position.market.closed ? "CLOSED" : "OPEN"} ${position.market.status ? `(${position.market.status})` : ""}`);
  ...
  console.log(`   Unrealized P&L: $${formatUSDC(position.unrealizedPnl)} ${formatPercent(pnlPercent)}`);
...
async function main() {
  ...
  try {
    ...
    console.log(`Total Unrealized:   $${formatUSDC(summary.totalUnrealizedPnl)} ${formatPercent(summary.totalUnrealizedPnlPercent)}`);
    ...
    console.log(`\nBreakdown:`);
    console.log(`  CLOB: ${summary.breakdown.clob.positions} positions, $${formatUSDC(summary.breakdown.clob.value)} value`);
    console.log(`  AMM:  ${summary.breakdown.amm.positions} positions, $${formatUSDC(summary.breakdown.amm.value)} value`);
    console.log(`\nPoints:`);
